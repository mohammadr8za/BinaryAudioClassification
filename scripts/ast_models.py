...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=192):
        super().__init__()
        ...
        self.proj = nn.Conv2d(1, embed_dim, kernel_size=(4, 4), stride=patch_size)

    def forward(self, x):
        x = self.proj(x.squeeze(3)).flatten(2).transpose(1, 2)

        #         x = self.proj(x).flatten(2).transpose(1, 2)

        # x = self.proj(x.reshape(4096,1,16,5)).flatten(2).transpose(1, 2)
        return x
...
class ASTModel(nn.Module):
    ...
    def __init__(self, label_dim=527, fstride=2, tstride=2, input_fdim=128, input_tdim=1024, imagenet_pretrain=True,
                 audioset_pretrain=False, model_size='base384', verbose=True, drop_out_rate=0):
        ...
        if audioset_pretrain == False:
            if model_size == 'tiny224':
                self.v = timm.create_model('vit_deit_tiny_distilled_patch16_224', pretrained=imagenet_pretrain, drop_rate=drop_out_rate)
            elif model_size == 'small224':
                self.v = timm.create_model('vit_deit_small_distilled_patch16_224', pretrained=imagenet_pretrain, drop_rate=drop_out_rate)
            ...
            new_proj = torch.nn.Conv2d(1, self.original_embedding_dim, kernel_size=(4, 4), stride=(fstride, tstride))
        ...
        # now load a model that is pretrained on both ImageNet and AudioSet
            ...
            audio_model = ASTModel(label_dim=527, fstride=2, tstride=2, input_fdim=128, input_tdim=1024,
    ...
    def get_shape(self, fstride, tstride, input_fdim=128, input_tdim=1024):
        ...
        test_proj = nn.Conv2d(1, self.original_embedding_dim, kernel_size=(4, 4), stride=(fstride, tstride))

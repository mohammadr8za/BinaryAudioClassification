...
def train(epoch, dataloader: torch.utils.data.DataLoader,
    ...
    for batch_idx, (inputs, targets) in enumerate(dataloader):
        ...
        # outputs, _ = model(inputs.type(torch.float), torch.tensor(10))
        outputs = model(inputs.type(torch.float))

        loss = loss_fn(outputs.squeeze().half(), targets)

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        loss_total.update(loss)
        accuracy.update(torch.argmax(outputs.squeeze(), dim=1), targets)
...
def valid(epoch, dataloader: torch.utils.data.DataLoader,
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            ...
            # outputs, _ = model(inputs.type(torch.float), 10)
            outputs = model(inputs.type(torch.float))
